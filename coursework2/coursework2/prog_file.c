...
void printHelp( void ) {
    ...
    char *helpText[] = {//----------------------------single line character limit----|
        "cThis program will apply a low-pass filter to audio from an input wav file",
        "cand write the result to a specified output wav file.",
        "c",
        "lThe required argument format is:",
        "c",
        "c<input file path> <output file path> <cut-off frequency>",
        "c",
        "cThe input and output paths should NOT contain whitespace. The frequency",
        "cshould be an integer in Hz. The frequency range supported is from 1Hz up ",
        "cto just less than half the sample frequency of the selected input file.",
        "c",
        "c---",
        "c",
        "c-->> Optional arguments are now available! <<--",
        "c",
        "cFor optimum results these options should be included before the required",
        "carguements. Please consult your doctor before use.",
        "c",
        "cIn order to use the once-in-a-lifetime HIGHPASS filter mode please include:",
        "c",
        "c-h",
        "cor",
        "c--highpass",
        "c",
        "cTo get your hands on some never-seen-before WINDOWING options please use:",
        "c",
        "c-w <window specifier>",
        "cor",
        "c--window <window specifier>",
        "c",
        "lThe window specifier may be:",
        "c",
        "l   rect  = Rectangular window function",
        "l   bart  = Bartlett window function (default)",
        "l   hann  = Hanning window function",
        "l   hamm  = Hamming window function (This one's a corker! [Henry, 2017])",
        "l   black = Blackman window function",
        "c",
        "cThe window specifier should IMMEDIATELY follow the -w or --window flag.",
        "c",
        "cAnd finally, if you're flying with our luxuary package, you may also like",
        "cto modify the BUFFER SIZE! Don't worry, no pesky dynamic allocation here!",
        "cSimply enter:",
        "c",
        "c-b <desired size>",
        "cor",
        "c--buffersize <desired size>",
        "c",
        "cWith a <desired size> between 64 and 2048 samples! Note, you must use a",
        "cpower of two. 128 samples is the default, but 256 will blow you away!",
        "c",
        "cThe desired size should IMMEDIATELY follow the -b or --buffersize flag.",
        "c",
        "c---",
        "c",
        "lReferences:",
        "c",
        "lHenry, Craig, 2017: Personal correspondance with the author."
    };
